...
urban_area = ['Aberdeen City','Aberdeenshire', 'East Lothian', 'Edinburgh, City of', 'Midlothian', 'Highland','Shetland','Lothian','West Lothian','Fife','Glasgow City']
...
df_final['Group_mean'] = df_final.groupby(['GROUP'])['Population'].transform('mean')
df_final.Group_mean = df_final.Group_mean.astype('f')
...
kdims = ['SII', 'RII']
vdims = ['GEOGRAPHY_NAME', 'Group_mean', 'GROUP']
dimensions = {
    'SII' : dict(label='SII (Slope Inequality Index)', range=(1000, 10000)),
    'RII': dict(label='RII (Relative Inequality Index)', range=(0.1, 1.2)),
    'Group_mean': ('group_mean', 'Group_mean')
}
...
# Define options
opts = {'plot': dict(width=1000, height=600,tools=['hover'], size_index='Group_mean',
...
hvgapminder = (gapminder_ds({'Points': opts}) * text({'Text': text_opts})).relabel('Emergency hospitalizations and inequality indexes')
